...
summary = pd.DataFrame({
    ...
    'Cost with PV (DKK)': [
        total_cost_pv['spot_cost'].sum(),
        total_cost_pv['tariff_cost'].sum(),
        total_cost_pv['energy_tax'].sum(),
        total_cost_pv['total_hourly_cost'].sum()
    ],
    'Savings (DKK)': [
        total_cost_without_pv['spot_cost'].sum() - total_cost_pv['spot_cost'].sum(),
        total_cost_without_pv['tariffs_cost'].sum() - total_cost_pv['tariff_cost'].sum(),
        total_cost_without_pv['tax_cost'].sum() - total_cost_pv['energy_tax'].sum(),
        total_cost_without_pv['total_hourly_cost'].sum() - total_cost_pv['total_hourly_cost'].sum()
    ]
}, index=['Spot price cost', 'Tariff cost', 'Energy tax cost', 'Total cost'])
...
# Do something with shiny.


# ---------------------------
# Battery optimisation simulation
# ---------------------------

# Battery operation simulation
# def simulate_battery_operation(df, battery_capacity=146, max_charge_rate=29, max_discharge_rate=29):
#     df = df.copy()
#     df = df.sort_values(['Date', 'Hour']).reset_index(drop=True)
#     df['battery_state'] = 0.0
#     df['battery_action'] = 'idle'
#     battery_level = 0.0

#     # Calculate price percentiles for charge/discharge thresholds
#     low_price_threshold = df['DKK/kWh'].quantile(0.25)
#     high_price_threshold = df['DKK/kWh'].quantile(0.75)
...
#         # Decide action
#         if price <= low_price_threshold and battery_level < battery_capacity:
#             # Charge battery
#             charge_amount = min(max_charge_rate, battery_capacity - battery_level)
#             battery_level += charge_amount
#             df.at[i, 'battery_action'] = f'charge {charge_amount:.2f} kWh'
#         elif price >= high_price_threshold and battery_level > 0:
#             # Discharge battery
#             discharge_amount = min(max_discharge_rate, battery_level, consumption)
#             battery_level -= discharge_amount
#             df.at[i, 'battery_action'] = f'discharge {discharge_amount:.2f} kWh'
#         else:
#             df.at[i, 'battery_action'] = 'idle'

#         df.at[i, 'battery_state'] = battery_level

#     return df

# # Example usage:
# battery_simulation = simulate_battery_operation(merged_spot)
# print(battery_simulation[['Date', 'Hour', 'DKK/kWh', 'Consumption kWh', 'battery_action', 'battery_state']].head(24))


# def simulate_battery_operation_daily(df, battery_capacity=146, max_charge_rate=29, max_discharge_rate=29):
#     df = df.copy()
#     df = df.sort_values(['Date', 'Hour']).reset_index(drop=True)
#     df['battery_state'] = 0.0
#     df['battery_action'] = 'idle'
...
#         low_price_threshold = day_df['DKK/kWh'].quantile(0.25)
#         high_price_threshold = day_df['DKK/kWh'].quantile(0.75)
#         day_indices = day_df.index

#         for i in day_indices:
#             price = df.at[i, 'DKK/kWh']
#             consumption = df.at[i, 'Consumption kWh']
